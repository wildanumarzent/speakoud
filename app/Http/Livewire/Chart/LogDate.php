...
class LogDate extends Component
{
    ...
    public function render()
    {
        ...
        $logDate = $this->log->get()->groupBy(function($date) {
            return Carbon::parse($date->created_at)->format('Y-m-d'); // grouping by date
            });
        $chart = (new ColumnChartModel());

        foreach($logDate as $a => $date){
        $section = array_rand($color);
        $colors = $color[$section][array_rand($color[$section])];
        $logCount = $this->log->whereDate('created_at',$a)->count();
        $chart->addColumn($a,$logCount,$colors);
        }
        return view('livewire.chart.log-date',compact('chart'));
    }
}
