...
<div class="card">
    ...
    <form action="{{ !isset($data['nilai']) ? route('grades.nilai.store', ['id' => $data['kategori']->id]) : route('grades.nilai.update', ['id' => $data['nilai']->kategori_id, 'nilaiId' => $data['nilai']->id]) }}" method="POST">
        ...
        <div class="card-footer text-center">
          <a href="{{ route('grades.nilai', ['id' => $data['kategori']->id]) }}" class="btn btn-danger" title="klik untuk kembali ke list">Kembali</a>
          &nbsp;&nbsp;
          <button type="submit" class="btn btn-primary" title="klik untuk menyimpan">{{ isset($data['nilai']) ? 'Simpan perubahan' : 'Simpan' }}</button>
          &nbsp;&nbsp;
