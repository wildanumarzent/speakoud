    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header">
                ...
                Pilih Tipe
                <span class="font-weight-light">Template Materi</span>
                <br>
                </h5>
            ...
            <div class="modal-body">
                @foreach (config('addon.label.bahan_tipe') as $key => $value)
                    @if ($value['get'] != 'evaluasi-pengajar')
                        <a href="{{ route('template.bahan.create', ['id' => $data['materi']->id, 'type' => $value['get']]) }}" class="media text-body px-3" title="tambah bahan dengan tipe {{ $value['title'] }}">
                        ...
                        @if (!empty($value['child']))
                            ...
                            <a href="{{ route('template.bahan.create', ['id' => $data['materi']->id, 'type' => $value['get'], 'kategori' => $valT['get']]) }}" class="media text-body px-3" title="tambah bahan tipe quiz dengan kategori {{ $valT['title'] }}">
                                <div class="box-materi py-3">
                                    <div class="dot-circle"></div>
                                    <div class="media-body ml-3">
