...
else{this.activity=new TinCan.Activity(cfg.activity);}}},_initFromQueryString:function(url){this.log("_initFromQueryString");var i,prop,qsParams=TinCan.Utils.parseURL(url).params,lrsProps=["endpoint","auth"],lrsCfg={},activityCfg,contextCfg,extended=null;if(qsParams.hasOwnProperty("actor")){this.log("_initFromQueryString - found actor: "+qsParams.actor);try{this.actor=TinCan.Agent.fromJSON(qsParams.actor);delete qsParams.actor;}
...
return stmt;},sendStatement:function(stmt,callback){this.log("sendStatement");var self=this,lrs,statement=this.prepareStatement(stmt),rsCount=this.recordStores.length,i,msg,results=[],callbackWrapper,callbackResults=[];if(rsCount>0){if(typeof callback==="function"){callbackWrapper=function(err,xhr){var args;self.log("sendStatement - callbackWrapper: "+rsCount);if(rsCount>1){rsCount-=1;callbackResults.push({err:err,xhr:xhr});}
...
else{msg="[warning] sendStatement: No LRSs added yet (statement not sent)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}
if(typeof callback==="function"){callback.apply(this,[null,statement]);}}
return{statement:statement,results:results};},getStatement:function(stmtId,callback){this.log("getStatement");var lrs,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];return lrs.retrieveStatement(stmtId,{callback:callback});}
msg="[warning] getStatement: No LRSs added yet (statement not retrieved)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}},voidStatement:function(stmt,callback,options){this.log("voidStatement");var self=this,lrs,actor,voidingStatement,rsCount=this.recordStores.length,i,msg,results=[],callbackWrapper,callbackResults=[];if(stmt instanceof TinCan.Statement){stmt=stmt.id;}
...
else{msg="[warning] voidStatement: No LRSs added yet (statement not sent)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}
if(typeof callback==="function"){callback.apply(this,[null,voidingStatement]);}}
return{statement:voidingStatement,results:results};},getVoidedStatement:function(stmtId,callback){this.log("getVoidedStatement");var lrs,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];return lrs.retrieveVoidedStatement(stmtId,{callback:callback});}
msg="[warning] getVoidedStatement: No LRSs added yet (statement not retrieved)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}},sendStatements:function(stmts,callback){this.log("sendStatements");var self=this,lrs,statements=[],rsCount=this.recordStores.length,i,msg,results=[],callbackWrapper,callbackResults=[];if(stmts.length===0){if(typeof callback==="function"){callback.apply(this,[null,statements]);}}
...
else{msg="[warning] sendStatements: No LRSs added yet (statements not sent)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}
if(typeof callback==="function"){callback.apply(this,[null,statements]);}}}
return{statements:statements,results:results};},getStatements:function(cfg){this.log("getStatements");var queryCfg={},lrs,params,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];cfg=cfg||{};params=cfg.params||{};if(cfg.sendActor&&this.actor!==null){if(lrs.version==="0.9"||lrs.version==="0.95"){params.actor=this.actor;}
...
msg="[warning] getStatements: No LRSs added yet (statements not read)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}},getState:function(key,cfg){this.log("getState");var queryCfg,lrs,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];cfg=cfg||{};queryCfg={agent:(typeof cfg.agent!=="undefined"?cfg.agent:this.actor),activity:(typeof cfg.activity!=="undefined"?cfg.activity:this.activity)};if(typeof cfg.registration!=="undefined"){queryCfg.registration=cfg.registration;}
...
msg="[warning] getState: No LRSs added yet (state not retrieved)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}},setState:function(key,val,cfg){this.log("setState");var queryCfg,lrs,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];cfg=cfg||{};queryCfg={agent:(typeof cfg.agent!=="undefined"?cfg.agent:this.actor),activity:(typeof cfg.activity!=="undefined"?cfg.activity:this.activity)};if(typeof cfg.registration!=="undefined"){queryCfg.registration=cfg.registration;}
else if(this.registration!==null){queryCfg.registration=this.registration;}
if(typeof cfg.lastSHA1!=="undefined"){queryCfg.lastSHA1=cfg.lastSHA1;}
if(typeof cfg.callback!=="undefined"){queryCfg.callback=cfg.callback;}
return lrs.saveState(key,val,queryCfg);}
msg="[warning] setState: No LRSs added yet (state not saved)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}},deleteState:function(key,cfg){this.log("deleteState");var queryCfg,lrs,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];cfg=cfg||{};queryCfg={agent:(typeof cfg.agent!=="undefined"?cfg.agent:this.actor),activity:(typeof cfg.activity!=="undefined"?cfg.activity:this.activity)};if(typeof cfg.registration!=="undefined"){queryCfg.registration=cfg.registration;}
else if(this.registration!==null){queryCfg.registration=this.registration;}
if(typeof cfg.callback!=="undefined"){queryCfg.callback=cfg.callback;}
return lrs.dropState(key,queryCfg);}
msg="[warning] deleteState: No LRSs added yet (state not deleted)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}},getActivityProfile:function(key,cfg){this.log("getActivityProfile");var queryCfg,lrs,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];cfg=cfg||{};queryCfg={activity:(typeof cfg.activity!=="undefined"?cfg.activity:this.activity)};if(typeof cfg.callback!=="undefined"){queryCfg.callback=cfg.callback;}
return lrs.retrieveActivityProfile(key,queryCfg);}
msg="[warning] getActivityProfile: No LRSs added yet (activity profile not retrieved)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}},setActivityProfile:function(key,val,cfg){this.log("setActivityProfile");var queryCfg,lrs,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];cfg=cfg||{};queryCfg={activity:(typeof cfg.activity!=="undefined"?cfg.activity:this.activity)};if(typeof cfg.callback!=="undefined"){queryCfg.callback=cfg.callback;}
if(typeof cfg.lastSHA1!=="undefined"){queryCfg.lastSHA1=cfg.lastSHA1;}
return lrs.saveActivityProfile(key,val,queryCfg);}
msg="[warning] setActivityProfile: No LRSs added yet (activity profile not saved)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}},deleteActivityProfile:function(key,cfg){this.log("deleteActivityProfile");var queryCfg,lrs,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];cfg=cfg||{};queryCfg={activity:(typeof cfg.activity!=="undefined"?cfg.activity:this.activity)};if(typeof cfg.callback!=="undefined"){queryCfg.callback=cfg.callback;}
return lrs.dropActivityProfile(key,queryCfg);}
msg="[warning] deleteActivityProfile: No LRSs added yet (activity profile not deleted)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}}};TinCan.DEBUG=false;TinCan.enableDebug=function(){TinCan.DEBUG=true;};TinCan.disableDebug=function(){TinCan.DEBUG=false;};TinCan.versions=function(){return["1.0.0","0.95","0.9"];};TinCan.environment=function(){if(_environment===null){_environment={};if(typeof window!=="undefined"){_environment.isBrowser=true;_environment.hasCORS=false;_environment.useXDR=false;if(typeof XMLHttpRequest!=="undefined"&&typeof(new XMLHttpRequest()).withCredentials!=="undefined"){_environment.hasCORS=true;}
else if(typeof XDomainRequest!=="undefined"){_environment.hasCORS=true;_environment.useXDR=true;}}
else{_environment.isBrowser=false;}}
return _environment;};if(TinCan.environment().isBrowser){if(!window.JSON){window.JSON={parse:function(sJSON){return eval("("+sJSON+")");},stringify:function(vContent){var sOutput="",nId,sProp;if(vContent instanceof Object){if(vContent.constructor===Array){for(nId=0;nId<vContent.length;nId+=1){sOutput+=this.stringify(vContent[nId])+",";}
...
return{path:parts[0],params:params};},getServerRoot:function(absoluteUrl){var urlParts=absoluteUrl.split("/");return urlParts[0]+"//"+urlParts[2];}};}());(function(){"use strict";var XDR="xdr",NATIVE="native",LRS=TinCan.LRS=function(cfg){this.log("constructor");this.endpoint=null;this.version=null;this.auth=null;this.allowFail=true;this.alertOnRequestFailure=true;this.extended=null;this._requestMode=NATIVE;this.init(cfg);};LRS.prototype={LOG_SRC:"LRS",log:TinCan.prototype.log,init:function(cfg){this.log("init");var urlParts,schemeMatches,locationPort,isXD,env=TinCan.environment(),versions=TinCan.versions(),versionMatch=false,i;cfg=cfg||{};if(cfg.hasOwnProperty("alertOnRequestFailure")){this.alertOnRequestFailure=cfg.alertOnRequestFailure;}
if(!cfg.hasOwnProperty("endpoint")){if(env.isBrowser&&this.alertOnRequestFailure){alert("[error] LRS invalid: no endpoint");}
throw{code:3,mesg:"LRS invalid: no endpoint"};}
this.endpoint=cfg.endpoint;if(this.endpoint.slice(-1)!=="/"){this.log("adding trailing slash to endpoint");this.endpoint+="/";}
...
if(env.isBrowser){urlParts=this.endpoint.toLowerCase().match(/([A-Za-z]+:)\/\/([^:\/]+):?(\d+)?(\/.*)?$/);if(urlParts===null){if(this.alertOnRequestFailure){alert("[error] LRS invalid: failed to divide URL parts");}
...
isXD=(!schemeMatches||location.hostname.toLowerCase()!==urlParts[2]||locationPort!==((urlParts[3]!==null&&typeof urlParts[3]!=="undefined"&&urlParts[3]!=="")?urlParts[3]:(urlParts[1]==="http:"?"80":(urlParts[1]==="https:"?"443":""))));if(isXD){if(env.hasCORS){if(env.useXDR&&schemeMatches){this._requestMode=XDR;}
else if(env.useXDR&&!schemeMatches){if(cfg.allowFail){if(this.alertOnRequestFailure){alert("[warning] LRS invalid: cross domain request for differing scheme in IE with XDR");}}
else{if(this.alertOnRequestFailure){alert("[error] LRS invalid: cross domain request for differing scheme in IE with XDR");}
throw{code:2,mesg:"LRS invalid: cross domain request for differing scheme in IE with XDR"};}}}
else{if(cfg.allowFail){if(this.alertOnRequestFailure){alert("[warning] LRS invalid: cross domain requests not supported in this browser");}}
else{if(this.alertOnRequestFailure){alert("[error] LRS invalid: cross domain requests not supported in this browser");}
throw{code:1,mesg:"LRS invalid: cross domain requests not supported in this browser"};}}}}
else{this.log("Unrecognized environment not supported: "+env);}
if(typeof cfg.version!=="undefined"){this.log("version: "+cfg.version);for(i=0;i<versions.length;i+=1){if(versions[i]===cfg.version){versionMatch=true;break;}}
if(!versionMatch){if(env.isBrowser&&this.alertOnRequestFailure){alert("[error] LRS invalid: version not supported ("+cfg.version+")");}
throw{code:5,mesg:"LRS invalid: version not supported ("+cfg.version+")"};}
this.version=cfg.version;}
else{this.version=versions[0];}},sendRequest:function(cfg){this.log("sendRequest");var xhr,finished=false,location=window.location,fullUrl=this.endpoint+cfg.url,headers={},data,requestCompleteResult,until,prop,pairs=[],self=this;function requestComplete(){self.log("requestComplete: "+finished+", xhr.status: "+xhr.status);var notFoundOk,httpStatus;httpStatus=(xhr.status===1223)?204:xhr.status;if(!finished){finished=true;notFoundOk=(cfg.ignore404&&httpStatus===404);if(httpStatus===undefined||(httpStatus>=200&&httpStatus<400)||notFoundOk){if(cfg.callback){cfg.callback(null,xhr);}
else{requestCompleteResult={err:null,xhr:xhr};return requestCompleteResult;}}
else{if(httpStatus>0){requestCompleteResult={err:httpStatus,xhr:xhr};if(self.alertOnRequestFailure){alert("[warning] There was a problem communicating with the Learning Record Store. ("+httpStatus+" | "+xhr.responseText+")");}
if(cfg.callback){cfg.callback(httpStatus,xhr);}}
return requestCompleteResult;}}
else{return requestCompleteResult;}}
if(cfg.url.indexOf("http")===0){fullUrl=cfg.url;}
...
headers["Content-Type"]="application/json";headers.Authorization=this.auth;if(this.version!=="0.9"){headers["X-Experience-API-Version"]=this.version;}
for(prop in cfg.headers){if(cfg.headers.hasOwnProperty(prop)){headers[prop]=cfg.headers[prop];}}
if(this._requestMode===NATIVE){this.log("sendRequest using XMLHttpRequest");for(prop in cfg.params){if(cfg.params.hasOwnProperty(prop)){pairs.push(prop+"="+encodeURIComponent(cfg.params[prop]));}}
if(pairs.length>0){fullUrl+="?"+pairs.join("&");}
this.log("sendRequest using XMLHttpRequest - async: "+(typeof cfg.callback!=="undefined"));if(typeof XMLHttpRequest!=="undefined"){xhr=new XMLHttpRequest();}
else{xhr=new ActiveXObject("Microsoft.XMLHTTP");}
xhr.open(cfg.method,fullUrl,(typeof cfg.callback!=="undefined"));for(prop in headers){if(headers.hasOwnProperty(prop)){xhr.setRequestHeader(prop,headers[prop]);}}
if(typeof cfg.data!=="undefined"){cfg.data+="";}
data=cfg.data;xhr.onreadystatechange=function(){self.log("xhr.onreadystatechange - xhr.readyState: "+xhr.readyState);if(xhr.readyState===4){requestComplete();}};}
else if(this._requestMode===XDR){this.log("sendRequest using XDomainRequest");fullUrl+="?method="+cfg.method;for(prop in cfg.params){if(cfg.params.hasOwnProperty(prop)){pairs.push(prop+"="+encodeURIComponent(cfg.params[prop]));}}
...
data=pairs.join("&");xhr=new XDomainRequest();xhr.open("POST",fullUrl);xhr.onload=function(){requestComplete();};xhr.onerror=function(){requestComplete();};}
else{this.log("sendRequest unrecognized _requestMode: "+this._requestMode);}
try{xhr.send(data);}
catch(ex){this.log("sendRequest caught send exception: "+ex);}
if(!cfg.callback){if(this._requestMode===XDR){until=1000+Date.now();this.log("sendRequest - until: "+until+", finished: "+finished);while(Date.now()<until&&!finished){this.__delay();}}
return requestComplete();}
return xhr;},saveStatement:function(stmt,cfg){this.log("saveStatement");var requestCfg;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
cfg=cfg||{};requestCfg={url:"statements",data:JSON.stringify(stmt.asVersion(this.version))};if(stmt.id!==null){requestCfg.method="PUT";requestCfg.params={statementId:stmt.id};}
else{requestCfg.method="POST";}
if(typeof cfg.callback!=="undefined"){requestCfg.callback=cfg.callback;}
return this.sendRequest(requestCfg);},retrieveStatement:function(stmtId,cfg){this.log("retrieveStatement");var requestCfg,requestResult,callbackWrapper;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
cfg=cfg||{};requestCfg={url:"statements",method:"GET",params:{statementId:stmtId}};if(typeof cfg.callback!=="undefined"){callbackWrapper=function(err,xhr){var result=xhr;if(err===null){result=TinCan.Statement.fromJSON(xhr.responseText);}
...
return requestResult;},retrieveVoidedStatement:function(stmtId,cfg){this.log("retrieveStatement");var requestCfg,requestResult,callbackWrapper;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
cfg=cfg||{};requestCfg={url:"statements",method:"GET",params:{}};if(this.version==="0.9"||this.version==="0.95"){requestCfg.params.statementId=stmtId;}
...
cfg.callback(err,result);};requestCfg.callback=callbackWrapper;}
requestResult=this.sendRequest(requestCfg);if(!callbackWrapper){requestResult.statement=null;if(requestResult.err===null){requestResult.statement=TinCan.Statement.fromJSON(requestResult.xhr.responseText);}}
return requestResult;},saveStatements:function(stmts,cfg){this.log("saveStatements");var requestCfg,versionedStatements=[],i;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
cfg=cfg||{};if(stmts.length===0){if(typeof cfg.callback!=="undefined"){cfg.callback.apply(this,["no statements"]);}
return;}
for(i=0;i<stmts.length;i+=1){versionedStatements.push(stmts[i].asVersion(this.version));}
requestCfg={url:"statements",method:"POST",data:JSON.stringify(versionedStatements)};if(typeof cfg.callback!=="undefined"){requestCfg.callback=cfg.callback;}
return this.sendRequest(requestCfg);},queryStatements:function(cfg){this.log("queryStatements");var requestCfg,requestResult,callbackWrapper;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
cfg=cfg||{};cfg.params=cfg.params||{};try{requestCfg=this._queryStatementsRequestCfg(cfg);}
catch(ex){if(TinCan.environment().isBrowser&&this.alertOnRequestFailure){alert("[error] Query statements failed - "+ex);}
if(typeof cfg.callback!=="undefined"){cfg.callback(ex,{});}
...
return returnCfg;},moreStatements:function(cfg){this.log("moreStatements: "+cfg.url);var requestCfg,requestResult,callbackWrapper,parsedURL,serverRoot;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
cfg=cfg||{};parsedURL=TinCan.Utils.parseURL(cfg.url);serverRoot=TinCan.Utils.getServerRoot(this.endpoint);if(parsedURL.path.indexOf("/statements")===0){parsedURL.path=this.endpoint.replace(serverRoot,'')+parsedURL.path;this.log("converting non-standard more URL to "+parsedURL.path);}
...
cfg.callback(err,result);};requestCfg.callback=callbackWrapper;}
requestResult=this.sendRequest(requestCfg);requestResult.config=requestCfg;if(!callbackWrapper){requestResult.statementsResult=null;if(requestResult.err===null){requestResult.statementsResult=TinCan.StatementsResult.fromJSON(requestResult.xhr.responseText);}}
return requestResult;},retrieveState:function(key,cfg){this.log("retrieveState");var requestParams={},requestCfg={},requestResult,callbackWrapper;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
...
return requestResult;},saveState:function(key,val,cfg){this.log("saveState");var requestParams,requestCfg,requestResult;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
if(typeof val==="object"){val=JSON.stringify(val);}
requestParams={stateId:key,activityId:cfg.activity.id};if(this.version==="0.9"){requestParams.actor=JSON.stringify(cfg.agent.asVersion(this.version));}
else{requestParams.agent=JSON.stringify(cfg.agent.asVersion(this.version));}
if(typeof cfg.registration!=="undefined"){if(this.version==="0.9"){requestParams.registrationId=cfg.registration;}
else{requestParams.registration=cfg.registration;}}
requestCfg={url:"activities/state",method:"PUT",params:requestParams,data:val};if(typeof cfg.callback!=="undefined"){requestCfg.callback=cfg.callback;}
if(typeof cfg.lastSHA1!=="undefined"&&cfg.lastSHA1!==null){requestCfg.headers={"If-Match":cfg.lastSHA1};}
return this.sendRequest(requestCfg);},dropState:function(key,cfg){this.log("dropState");var requestParams={},requestCfg={};if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
requestParams={activityId:cfg.activity.id};if(this.version==="0.9"){requestParams.actor=JSON.stringify(cfg.agent.asVersion(this.version));}
else{requestParams.agent=JSON.stringify(cfg.agent.asVersion(this.version));}
...
return this.sendRequest(requestCfg);},retrieveActivityProfile:function(key,cfg){this.log("retrieveActivityProfile");var requestCfg={},requestResult,callbackWrapper;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
requestCfg={url:"activities/profile",method:"GET",params:{profileId:key,activityId:cfg.activity.id},ignore404:true};if(typeof cfg.callback!=="undefined"){callbackWrapper=function(err,xhr){var result=xhr;if(err===null){if(xhr.status===404){result=null;}
...
return requestResult;},saveActivityProfile:function(key,val,cfg){this.log("saveActivityProfile");var requestCfg;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
if(typeof val==="object"){val=JSON.stringify(val);}
requestCfg={url:"activities/profile",method:"PUT",params:{profileId:key,activityId:cfg.activity.id},data:val};if(typeof cfg.callback!=="undefined"){requestCfg.callback=cfg.callback;}
if(typeof cfg.lastSHA1!=="undefined"&&cfg.lastSHA1!==null){requestCfg.headers={"If-Match":cfg.lastSHA1};}
else{requestCfg.headers={"If-None-Match":"*"};}
return this.sendRequest(requestCfg);},dropActivityProfile:function(key,cfg){this.log("dropActivityProfile");var requestParams={},requestCfg={};if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
requestParams={activityId:cfg.activity.id};if(key!==null){requestParams.profileId=key;}
requestCfg={url:"activities/profile",method:"DELETE",params:requestParams};if(typeof cfg.callback!=="undefined"){requestCfg.callback=cfg.callback;}
return this.sendRequest(requestCfg);},__delay:function(){var xhr=new XMLHttpRequest(),url=window.location+"?forcenocache="+TinCan.Utils.getUUID();xhr.open("GET",url,false);xhr.send(null);}};}());(function(){"use strict";var AgentAccount=TinCan.AgentAccount=function(cfg){this.log("constructor");this.homePage=null;this.name=null;this.init(cfg);};AgentAccount.prototype={LOG_SRC:'AgentAccount',log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,directProps=["name","homePage"],val;cfg=cfg||{};if(typeof cfg.accountServiceHomePage!=="undefined"){cfg.homePage=cfg.accountServiceHomePage;}
if(typeof cfg.accountName!=="undefined"){cfg.name=cfg.accountName;}
for(i=0;i<directProps.length;i+=1){if(cfg.hasOwnProperty(directProps[i])&&cfg[directProps[i]]!==null){this[directProps[i]]=cfg[directProps[i]];}}},toString:function(lang){this.log("toString");var result="";if(this.name!==null||this.homePage!==null){result+=this.name!==null?this.name:"-";result+=":";result+=this.homePage!==null?this.homePage:"-";}
...
this[directProps[i]]=val;}}},toString:function(lang){this.log("toString");if(this.name!==null){return this.name;}
...
if(this.completion==="Completed"){this.completion=true;}},asVersion:function(version){this.log("asVersion");var result={},optionalDirectProps=["success","duration","response","extensions"],optionalObjProps=["score"],i,prop;version=version||TinCan.versions()[0];for(i=0;i<optionalDirectProps.length;i+=1){if(this[optionalDirectProps[i]]!==null){result[optionalDirectProps[i]]=this[optionalDirectProps[i]];}}
...
return result;}};Activity.fromJSON=function(activityJSON){Activity.prototype.log("fromJSON");var _activity=JSON.parse(activityJSON);return new Activity(_activity);};}());(function(){"use strict";var ContextActivities=TinCan.ContextActivities=function(cfg){this.log("constructor");this.parent=null;this.grouping=null;this.other=null;this.init(cfg);};ContextActivities.prototype={LOG_SRC:'ContextActivities',log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,j,objProps=["parent","grouping","other"],prop,val;cfg=cfg||{};for(i=0;i<objProps.length;i+=1){prop=objProps[i];if(cfg.hasOwnProperty(prop)&&cfg[prop]!==null){if(Object.prototype.toString.call(cfg[prop])==="[object Array]"){for(j=0;j<cfg[prop].length;j+=1){this.add(prop,cfg[prop][j]);}}
else{val=cfg[prop];this.add(prop,val);}}}},add:function(key,val){if(key!=="parent"&&key!=="grouping"&&key!=="other"){return;}
if(this[key]===null){this[key]=[];}
if(!(val instanceof TinCan.Activity)){val=typeof val==='string'?{id:val}:val;val=new TinCan.Activity(val);}
this[key].push(val);return this[key].length-1;},asVersion:function(version){this.log("asVersion");var result={},optionalObjProps=["parent","grouping","other"],i,j,prop;version=version||TinCan.versions()[0];for(i=0;i<optionalObjProps.length;i+=1){if(this[optionalObjProps[i]]!==null&&this[optionalObjProps[i]].length>0){if(version==="0.9"||version==="0.95"){if(this[optionalObjProps[i]].length>1){this.log("[WARNING] version does not support multiple values in: "+optionalObjProps[i]);}
...
(this.target!==null?this.target.toString(lang):"");},asVersion:function(version){this.log("asVersion");var result,optionalDirectProps=["timestamp"],optionalObjProps=["result","context"],i;version=version||TinCan.versions()[0];result={objectType:this.objectType,actor:this.actor.asVersion(version),verb:this.verb.asVersion(version),object:this.target.asVersion(version)};for(i=0;i<optionalDirectProps.length;i+=1){if(this[optionalDirectProps[i]]!==null){result[optionalDirectProps[i]]=this[optionalDirectProps[i]];}}
for(i=0;i<optionalObjProps.length;i+=1){if(this[optionalObjProps[i]]!==null){result[optionalObjProps[i]]=this[optionalObjProps[i]].asVersion(version);}}
...
return result;}
function TCAPI_GetStudentID(){WriteToDebug("In TCAPI_GetStudentID");return tincan.actor.mbox;}
